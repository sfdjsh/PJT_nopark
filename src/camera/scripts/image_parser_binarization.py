...
class IMGParser:
    ...
    def callback(self, msg):
        ...
        lower_ylane = np.array([10, 87, 151])
        upper_ylane = np.array([21, 179, 223])
        ...
        img_wresult = cv2.bitwise_and(img_bgr, img_bgr, mask=img_wlane)
        img_ylane = cv2.inRange(img_hsv, lower_ylane, upper_ylane)
        img_yresult = cv2.bitwise_and(img_bgr, img_bgr, mask=img_ylane)
