            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses :
                    if global_vaild_object[i][0]==1 or global_vaild_object[i][0]==2 :  
                        pass
                        dis = sqrt(abs(local_vaild_object[i][1])**2 + abs(local_vaild_object[i][1]**2))
                        if dis < 35:
                            rel_distance = sqrt(abs(path.pose.position.x - global_vaild_object[i][1])**2 + abs(path.pose.position.y - global_vaild_object[i][2]))                         
