...
class lc_path_pub :
    def __init__(self):
        ...
        # 읽어 온 경로 데이터를 Global Path 변수에 넣기
    ...
    def getLaneChangePath(self,ego_path,lc_path,start_point,end_point,start_next_point, end_waypoint_idx): ## 
        ...
        start_path_distance = sqrt(abs(start_point.pose.position.x - end_point.pose.position.x)**2
        + abs(start_point.pose.position.y - end_point.pose.position.y)**2)
        ...
        for k in range(0,start_path_repeat+1):
            ...
            read_pose=PoseStamped()
            read_pose.pose.position.x = start_point.pose.position.x + roation_matric_calc[0] # 그럼 ego_path? 아니면 pose?
            read_pose.pose.position.y = start_next_point.pose.position.y + roation_matric_calc[1]
